        ...
        # 4. html parse - get site reservation  info
            ...
            for i in range(len(tags)):
                ...
                if txt.find('doDraw') > -1:
                    ...
                    # Future day check  AND CHECK_DAY check - Friday (4) , Saturday (5),  Sunday (6) AND HOLYDAY Check
                    
                    if date_diff.days > 0 and (config.CHECK_DAY.find(str(dw)) > -1 or config.HOLYDAY.find(reservation_date.strftime('%Y-%m-%d')) > -1) and notPreCheckAndExceptionCheck(reservation_date.strftime('%Y-%m-%d')+":"+date_info[3].replace("'","" ),site_name) :
